...
class DiscordWrapper:
        ...
        if str(message.author) != str(self.bot.user):
            ...
            if not channel:
                if content.startswith("@"):
                    ...
                    content = (
                        "I do not know who you want to contact, "
                        + "please prefix your message with @User"
                    )
    ...
    async def on_irc_message(self, **kwargs):
        """Event on IRC message received"""
        maps = {irc: discord for discord, irc in self.channels.items()}
        message = kwargs["data"]
