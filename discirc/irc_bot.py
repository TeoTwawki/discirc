...
class IRCBot(bottom.Client):
    ...
    def on_irc_message(self, nick: str, target: str, message: str):
    ...
    def on_discord_message(self, **kwargs):
        ...
        for msg in content.split("\n"):
            ...
            color = self.COLOR_PREF + str(
                sum([ord(letter) for letter in source]) % 15
            )
            format_msg = "<{}{}{}> {}".format(color, source, self.CANCEL, msg)
            chunks = textwrap.wrap(
                format_msg, self.IRC_MSG_CHUNK_LENGTH, break_long_words=False
            )
            for chunk in chunks:
